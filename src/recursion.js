...
var createArray = function(str) {
  if(str.length === 1) {
    ...
    return createArray(str.slice(0,1)).concat(createArray(str.slice(1)));
  }
};
...
var buildList = function(value, length) {
  if (length === 1) {
    ...
    return buildList(value,1).concat(buildList(value,length-1));
  }
};
...
var fizzBuzz = function(n) {
  if (n === 1) {
    ...
    return fizzBuzz(n-1).concat(last);
  }
};
...
var rMap = function(array, callback) {
  if (array.length === 1) {
    // return [ callback(array[0]) ]; //equivalent
    // return [ callback.call(null, array[0]) ]; //equivalent
    return [ callback.apply(null, array) ]; //equivalent
  } else {
...
var countValuesInObj = function(obj, value) {
  ...
  for (let i = 0; i < Object.values(obj).length; i++) {
    if (Object.values(obj)[i] === value) {
      ...
      count += countValuesInObj(Object.values(obj)[i], value);
    }
  }
  return count;
};
...
var fibonacci = function(n) {
  if (n <= 0) {
    ...
    let subsequentNum = arr.slice(-2).reduce( (secondtolast, last) => secondtolast + last );
    return [...arr, subsequentNum];
  }
    // arr.push(arr[arr.length -1] + arr[arr.length - 2]); // also works
    // arr.concat(arr[arr.length -1] + arr[arr.length - 2]); // also works
    // return arr; // also works
...
var letterTally = function(str, obj) {
  if (str.length === 1 && typeof obj !== 'object') {
    obj = {}
    obj[str[0]] = 1;
  } else if (str.length === 1 && typeof obj === 'object' && !obj[str[0]]) {
    obj[str[0]] = 1;
  } else if (str.length === 1 && typeof obj === 'object' && obj[str[0]]) {
    obj[str[0]] += 1;
  } else {
    obj = letterTally(str.slice(1), letterTally(str.slice(0,1),obj));
  }
  return obj;
...
var compress = function(list) {
  if (list.length === 1) {
    return list;
    //base case
  } else if (list.length >= 2 && list[list.length - 1] === list[list.length - 2]) {
    return compress(list.slice(0,-1));
    // if repeat found at last two elements, return compressed array with last element sliced
  } else if (list.length >= 2 && list[list.length - 1] !== list[list.length - 2]) {
    return compress(list.slice(0,-1)).concat(list.slice(-1));
    // if no repeat found at last two elements, return compress array with last element sliced, but keep and concatenate that last element
  }
...
var minimizeZeroes = function(array) {
  if (array.length === 1) {
    ...
    return minimizeZeroes(array.slice(0,-1));
    // if repeat zero found at last two elements,
    // return minimizeZeroes array with last element sliced
  } else {
    return minimizeZeroes(array.slice(0,-1)).concat(array.slice(-1));
    // if no repeat zero found at last two elements,
    // return minimizeZeroes array with last element sliced, but keep and concatenate that last element
  }
...
// alternateSign([-2,-7,8,3,-1,4,5]) // [2,-7,8,-3,1,-4,5]
// test missing in SpecRunner for arrays with odd length
