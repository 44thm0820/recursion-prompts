...
var sum = function(array) {
  if (array.length === 0) {
    ...
    return sum(array.slice(0,1)) + sum(array.slice(1));
  }
};
...
var range = function(x, y) {
  if (x > y) {
    return range(y, x).slice().reverse();
  } else if (y - x < 2) {
    return [];
  } else if (y - x === 2) { //smallest array gives base case
    return [x + 1]
  } else {
...
var createArray = function(str) {
  if (str.length === 1) {
    ...
    return createArray(str[0]).concat(createArray(str.slice(1)));
  }
};
